...
# Phase 9 → SQLite retirement (2026-04-29)
...
#   Empty value fails-loud at pool init.
...
# OFFLINE_DEMO: when true, external dependencies are bypassed for offline pitch
# fallback. Skips Marengo / Claude SDK pre-warm and Sentry init; flips
# STORAGE_BACKEND to "local" (D-18). Does NOT affect DB routing — there is now
# only one DB backend (Postgres on Neon). Setting OFFLINE_DEMO=true without a
# reachable DATABASE_URL will fail at lifespan startup; that's by design.
